...
def evaluate_transformer_model(config):
    model = TransformerModel(model_type='bert',
                             model_name='bert-base-uncased',  # config['paths']['models'] + 'ner/trf_bert_2',
    ...
    performance = model.test(config['paths']['data'] + 'ner/manual_training_data/per_loc_validation_1.csv',
                             output_file='model_evaluation/ner/trf_bert_0.txt')
    safe_predictions_to_csv(to='model_evaluation/ner/trf_bert_0.csv', prediction_results=performance)
