...
def evaluate_transformer_model(config):
    model = TransformerModel(model_type='bert',
                             model_name=config['paths']['models'] + 'ner/trf_bert_4',
                             numbers_of_gpus=config['number_of_gpus'],
                             training_iterations=1,
                             gpu_id=config['gpu_id'])
    performance = model.test(config['paths']['data'] + 'ner/manual-validation-1.tsv',
                             output_file='model_evaluation/ner/trf_bert_4.txt')
    safe_predictions_to_csv(to='model_evaluation/ner/trf_bert_4.csv', prediction_results=performance)
